...
Для отримання повної інформації про API профілю AppArmor див. [Посилання на API](#api-reference).
...
```shell
kubectl exec <ім'я_пода> -- cat /proc/1/attr/current
```
...
`type` *(обов'язково)* — вказує, який тип профілю AppArmor буде застосований. Дійсні варіанти:
