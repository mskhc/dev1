...
Ви можете встановити квоту на загальну кількість певних ресурсів всіх стандартних, іменованих типів ресурсів за допомогою наступного синтаксису:
...
Той самий синтаксис можна використовувати для власних ресурсів. Наприклад, щоб створити квоту на власний ресурс `widgets` у групі API `example.com`, використовуйте `count/widgets.example.com`.

При використанні квоти на ресурс `count/*`, обʼєкт стягується з квоти, якщо він існує в сховищі сервера. Ці типи квот корисні для захисту від вичерпання ресурсів зберігання. Наприклад, ви можете хотіти обмежити кількість Secretʼів на сервері, враховуючи їх великий розмір. Занадто багато Secretʼів у кластері може фактично завадити запуску серверів і контролерів. Ви можете встановити квоту для Job, щоб захиститися від погано налаштованого CronJob. CronJobs, які створюють занадто багато Job у просторі імен, можуть призвести до заподіяння відмови в обслуговуванні.

Також можливо встановити загальну квоту на кількість обʼєктів для обмеженого набору ресурсів. Підтримуються наступні типи:
...
| `services.nodeports` | Загальна кількість Services типу `NodePort`, які можуть існувати в просторі імен. |
...
Коли до квоти додається сфера, вона обмежує кількість ресурсів, які вона підтримує, тими, які стосуються до цієї сфера. Ресурси, вказані у квоті поза дозволеним набором, призводять до помилки перевірки.
...
Сфера `BestEffort` обмежує квоту відстеження наступним ресурсом:

- `pods`

Сфери `Terminating`, `NotTerminating`, `NotBestEffort` та `PriorityClass` обмежують квоту відстеження наступними ресурсами:
...
Зверніть увагу, що ви не можете вказати як `Terminating`, так і `NotTerminating` сфери в одній й тій же квоті, і ви також не можете вказати як `BestEffort`, так і `NotBestEffort` сфери в одній й тій же квоті.

Селектор сфери підтримує наступні значення у полі `operator`:
...
- Для кожного пріоритету створюється один об'єкт квоти.
...
У цьому випадку створення Podʼу буде дозволено, якщо:

1. Параметр `priorityClassName` Podʼу не вказано.
1. Параметр `priorityClassName` Podʼу вказано на значення, відмінне від `cluster-services`.
1. Параметр `priorityClassName` Podʼу встановлено на `cluster-services`, він має бути створений в просторі імен `kube-system` і пройти перевірку обмеження ресурсів.

Запит на створення Podʼу буде відхилено, якщо його `priorityClassName` встановлено на `cluster-services` і він має бути створений в просторі імен, відмінному від `kube-system`.
