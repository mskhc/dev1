...
{{< note >}}
cpu_ids у `ContainerResources` у кінцевій точці `List` відповідають виключно виділеним ЦП, призначеним для певного контейнера. Якщо мета — оцінити ЦП, які належать до загального пула, кінцеву точку `List` необхідно використовувати разом з кінцевою точкою `GetAllocatableResources`, як пояснено
нижче:
...
### Кінцева точка gRPC `GetAllocatableResources` {#grpc-endpoint-getallocatableresources}

{{< feature-state state="stable" for_k8s_version="v1.28" >}}

Кінцева точка `GetAllocatableResources` надає інформацію про ресурси, що спочатку доступні на робочому вузлі. Вона надає більше інформації, ніж kubelet експортує в APIServer.

{{< note >}}
`GetAllocatableResources` слід використовувати лише для оцінки [виділених](/docs/tasks/administer-cluster/reserve-compute-resources/#node-allocatable) ресурсів на вузлі. Якщо мета - оцінити вільні/невикористані ресурси, її слід використовувати разом з кінцевою точкою List(). Результат, отриманий за допомогою `GetAllocatableResources`, залишиться таким самим, якщо ресурси, які експонуються для kubelet, не змінюються. Це відбувається рідко, але коли це відбувається (наприклад: hotplug/hotunplug, зміни стану пристроїв), очікується, що клієнт викличе кінцеву точку `GetAllocatableResources`.

Однак виклик кінцевої точки `GetAllocatableResources` не є достатнім у випадку оновлення ЦП та/або памʼяті, і для відображення правильної кількості та виділення ресурсів Kubelet повинен бути перезавантажений.
{{< /note >}}
...
### Кінцева точка gRPC `Get` {#grpc-endpoint-get}

{{< feature-state state="alpha" for_k8s_version="v1.27" >}}

Кінцева точка `Get` надає інформацію про ресурси робочого Pod. Вона експонує інформацію, аналогічну тій, що описана в кінцевій точці `List`. Кінцева точка `Get` вимагає `PodName` і `PodNamespace` робочого Pod.
...
Кінцева точка `Get` може надавати інформацію про Pod, повʼязану з динамічними ресурсами, виділеними за допомогою API динамічного виділення ресурсів. Для включення цієї функції вам потрібно забезпечити, щоб ваші служби kubelet були запущені з наступними прапорцями:
