...
{{< note >}}
Для використання цієї функціональності слід увімкнути [feature gate](/uk/docs/reference/command-line-tools-reference/feature-gates/)  `PodDisruptionConditions` у вашому кластері.
{{< /note >}}

При увімкненні цієї функціональності для Podʼа додається окрема умова `DisruptionTarget` [condition](/uk/docs/concepts/workloads/pods/pod-lifecycle/#pod-conditions), яка вказує, що Pod має бути видалений через {{<glossary_tooltip term_id="disruption" text="розлад">}}. Поле `reason` умови додатково вказує на одну з наступних причин завершення роботи Podʼа:
...
: Pod був примусово завершений kubelet, через {{<glossary_tooltip term_id="node-pressure-eviction" text="виселення через тиск вузла">}} або [відповідне вимикання вузла](/uk/docs/concepts/architecture/nodes/#graceful-node-shutdown).
...
При увімкненні функціональності `PodDisruptionConditions` разом із видаленням Podʼів, збирач сміття подів (PodGC) також відзначатиме їх як неуспішні, якщо вони знаходяться в не фазі завершення роботи (див. також [Збирання сміття Podʼів](/uk/docs/concepts/workloads/pods/pod-lifecycle/#pod-garbage-collection)).
