...
2. Якщо часи створення Podʼів відрізняються, то Pod, створений недавно, йде перед старішим Podʼом (часи створення розділені на цілочисельний логарифмічний масштаб, коли увімкнено [feature gate](/uk/docs/reference/command-line-tools-reference/feature-gates/) `LogarithmicScaleDown`)
...
Ця функція є бета-версією та увімкнена типов. Ви можете вимкнути її, використовуючи [feature gate](/uk/docs/reference/command-line-tools-reference/feature-gates/) `PodDeletionCost` як в kube-apiserver, так і в kube-controller-manager.
