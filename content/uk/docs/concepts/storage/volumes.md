...
### cephfs (застаріло) {#cephfs}

{{< feature-state for_k8s_version="v1.28" state="deprecated" >}}

{{< note >}}
Проєкт Kubernetes рекомендує натомість використовувати сторонній драйвер сховища [CephFS CSI](https://github.com/ceph/ceph-csi).
{{< /note >}}

Том `cephfs` дозволяє монтувати наявний том CephFS У ваш Pod. На відміну від `emptyDir`, який видаляється, коли Pod видаляється, вміст тому `cephfs` зберігається, і том лише розмонтовується. Це означає, що том `cephfs` може бути наперед наповнений даними, і ці дані можуть бути спільно використані між Podʼами. Том `cephfs` може бути змонтований кількома процесами запису одночасно.

{{< note >}}
Перед використанням вам потрібно мати власний сервер Ceph, що працює, з експортом розділу.
{{< /note >}}

Дивіться [приклад CephFS](https://github.com/kubernetes/examples/tree/master/volumes/cephfs/) для отримання докладнішої інформації.
...
[ConfigMap](/uk/docs/tasks/configure-pod-container/configure-pod-configmap/) надає можливість вводити конфігураційні дані у Podʼи. Дані, збережені в ConfigMap, можуть бути використані у томі типу `configMap` і потім використовуватись контейнеризованими застосунками, що працюють у Podʼі.
...
* Ви повинні [створити ConfigMap](/uk/docs/tasks/configure-pod-container/configure-pod-configmap/#create-a-configmap), перш ніж ви зможете його використовувати.
...
Дивіться [Передавання інформації про Podʼи контейнерам через файли](/uk/docs/tasks/inject-data-application/downward-api-volume-expose-pod-information/) для отримання докладнішої інформації.
...
Можна вказати обмеження розміру для типового носія, яке обмежує місткість тому `emptyDir`. Простір виділяється з [ефемерного сховища вузла](/uk/docs/concepts/configuration/manage-resources-containers/#setting-requests-and-limits-for-local-ephemeral-storage). Якщо він заповнюється з іншого джерела (наприклад, файли логів чи образи), том `emptyDir` може вийти з ладу через це обмеження.

{{< note >}}
Ви можете вказати розмір для томів, що зберігаються в памʼяті, за умови, що [фцнкціональну можливість](/uk/docs/reference/command-line-tools-reference/feature-gates/) `SizeMemoryBackedVolumes` увімкнено у вашому кластері (вона була бета-версією та типово активною починаючи з випуску Kubernetes 1.22). Якщо ви не вказуєте розмір тому, томи, що зберігаються в памʼяті, налаштовуються на обсяг доступної памʼяті вузла.
{{< /note >}}

{{< caution >}}
Будь ласка, перевірте [тут](/uk/docs/concepts/configuration/manage-resources-containers/#memory-backed-emptydir) пункти, які слід врахувати з точки зору управління ресурсами при використанні `emptyDir`, що зберігаються в памʼяті.
{{< /caution >}}
...
{{< warning >}}
Тип тому `gitRepo` застарів. Щоб забезпечити те, що Pod має змонтований репозиторій git,  ви можете змонтувати том [EmptyDir](#emptydir), який монтується в [init container](/uk/docs/concepts/workloads/pods/init-containers/), що клонує репозиторій за допомогою git, а потім монтує [EmptyDir](#emptydir) в контейнер Podʼа.

---

Ви можете обмежити використання томів `gitRepo` у вашому кластері, використовуючи [політики](/uk/docs/concepts/policy/), такі як [ValidatingAdmissionPolicy](/uk/docs/reference/access-authn-authz/validating-admission-policy/). Ви можете використовувати наступний вираз мови виразів загального виразу (Common Expression Language, CEL) як частину політики для відхилення використання томів `gitRepo`: `has(object.spec.volumes) || !object.spec.volumes.exists(v, has(v.gitRepo))`.
...
Деякі файли або каталоги, створені на базових вузлах, можуть бути доступні лише користувачу root. В такому випадку вам слід або запускати свій процес як root у [привілейованому контейнері](/uk/docs/tasks/configure-pod-container/security-context/) або змінювати права доступу до файлів на вузлі щоб мати змогу читати (або записувати) у том `hostPath`.
...
Обʼєкт `persistentVolumeClaim` використовується для монтування [PersistentVolume](/uk/docs/concepts/storage/persistent-volumes/) в Pod. PersistentVolumeClaim є способом для користувачів "вимагати" надійне сховище (наприклад, том iSCSI) без знання деталей конкретного хмарного середовища.

Дивіться інформацію щодо [PersistentVolumes](/uk/docs/concepts/storage/persistent-volumes/) для отримання додаткових деталей.
...
Міграційний механізм `CSIMigration` для Portworx був доданий, але він є типово вимкненим в Kubernetes 1.23, оскільки він перебуває в альфа-стані. З версії v1.25 він є бета-функцією, але типово вимкнений. Він перенаправляє всі операції втулка з існуючого вбудованого втулка на `pxd.portworx.com` Container Storage Interface (CSI) Driver. [Драйвер CSI для Portworx](https://docs.portworx.com/portworx-enterprise/operations/operate-kubernetes/storage-operations/csi) має бути встановленим у кластері. Щоб увімкнути цю функцію, встановіть `CSIMigrationPortworx=true` в kube-controller-manager та kubelet.

### projected

Том projected підʼєднує кілька існуючих джерел томів в одну теку. Докладніше дивіться [projected томи](/uk/docs/concepts/storage/projected-volumes/).
...
{{< note >}}
Проєкт Kubernetes рекомендує використовувати [Ceph CSI](https://github.com/ceph/ceph-csi) як зовнішній сторонній драйвер для зберігання в режимі RBD.
{{< /note >}}

Том `rbd` дозволяє підключати обʼєкт блочного пристрою [Rados](https://docs.ceph.com/en/latest/rbd/) (RBD) до вашого Podʼа. На відміну від `emptyDir`, який видаляється при видаленні Podʼа, вміст тому `rbd` зберігається, і том лише відмічається як відʼєднаний. Це означає, що том RBD можна наперед наповнити даними, і ці дані можна спільно використовувати між Podʼами.

{{< note >}}
Перш ніж використовувати RBD, вам потрібно мати встановлену систему Ceph.
{{< /note >}}

Особливістю RBD є те, що його можна підключити як лише для читанн одночасно для кількох споживачів. Це означає, що ви можете наперед наповнити том своїм набором даних, а потім використовувати його паралельно з будь-якою кількістю Podʼів. Недолік полягає в тому, що томи RBD можна підключити лише одним споживачем в режимі читання-запису. Одночасні записи від кількох акторів не дозволяються.

Докладніше дивіться в [прикладі RBD](https://github.com/kubernetes/examples/tree/master/volumes/rbd).

#### Міграція на CSI для RBD {#rbd-csi-migration}

{{< feature-state for_k8s_version="v1.28" state="deprecated" >}}

Функціональність `CSIMigration` для `RBD`, коли вона увімкнена, перенаправляє всі операції втулка з наявного вбудованого втулка на драйвер {{< glossary_tooltip text="CSI" term_id="csi" >}} `rbd.csi.ceph.com`. Щоб використовувати цю функцію, драйвер [Ceph CSI](https://github.com/ceph/ceph-csi) повинен бути встановлений в кластері Kubernetes, а [feature gate](/uk/docs/reference/command-line-tools-reference/feature-gates/) `CSIMigrationRBD` повинен бути увімкнений (Зауважте, що у релізі v1.24 прапорець `csiMigrationRBD` був видалений і замінений на `CSIMigrationRBD`).

{{< note >}}

Як оператору кластера Kubernetes, який керує сховищем, ось передумови, які вам слід виконати, перш ніж ви спробуєте виконати міграцію на драйвер CSI для RBD:

* Вам потрібно встановити Ceph CSI driver (`rbd.csi.ceph.com`), версії 3.5.0 чи вище, у вашому кластері Kubernetes.
* З урахуванням того, що поле `clusterID` є обовʼязковим параметром для драйвера CSI для його операцій, але вбудований StorageClass має поле `monitors` як обовʼязковий параметр, адміністратор зберігання Kubernetes повинен створити `clusterID` на основі хешу моніторів (наприклад: `echo -n '<monitors_string>' | md5sum`) у конфігурації CSI ConfigMap та тримати монітори в цій конфігурації `clusterID`.
* Також, якщо значення `adminId` у вбудованому StorageClass відрізняється від `admin`, то `adminSecretName`, зазначене в у вбудованому StorageClass, повинно бути замінено значенням base64 параметра `adminId`, інакше цей крок можна пропустити.
...
Для отримання додаткової інформації дивіться [Налаштування Secret](/uk/docs/concepts/configuration/secret/).
...
Щоб дізнатися про те, як запросити місце за допомогою специфікації ресурсів, дивіться [як управляти ресурсами](/uk/docs/concepts/configuration/manage-resources-containers/).
...
* з [загальним ефемерним томом](/uk/docs/concepts/storage/ephemeral-volumes/#generic-ephemeral-volumes)
* з [CSI ефемерним томом](/uk/docs/concepts/storage/ephemeral-volumes/#csi-ephemeral-volumes), якщо драйвер підтримує це
...
* `nodeExpandSecretRef`: Посилання на secret, який містить конфіденційну інформацію для передачі драйверу CSI для завершення виклику `NodeExpandVolume`. Це поле є необовʼязковим і може бути порожнім, якщо не потрібен жоден secret. Якщо обʼєкт містить більше одного secret, передаються всі secretʼи. Коли ви налаштовуєте конфіденційні дані secret для розширення тому, kubelet передає ці дані через виклик `NodeExpandVolume()` драйверу CSI. Щоб використовувати поле `nodeExpandSecretRef`, ваш кластер повинен працювати з версією Kubernetes 1.25 або новіщою.
* Якщо ви використовуєте Kubernetes версії 1.25 або 1.26, вам потрібно увімкнути [feature gate](/uk/docs/reference/command-line-tools-reference/feature-gates/) з назвою `CSINodeExpandSecret` для кожного kube-apiserver та для kubelet на кожному вузлі. У версії Kubernetes 1.27 цю функцію типово ввімкнено і не потрібно явно використовувати feature gate. Також вам потрібно використовувати драйвер CSI, який підтримує або вимагає конфіденційні дані secret під час операцій зміни розміру зберігання, ініційованих вузлом. 
...
Ви можете налаштувати ваш [PersistentVolume/PersistentVolumeClaim із підтримкою блочний томів](/uk/docs/concepts/storage/persistent-volumes/#raw-block-volume-support) як зазвичай, без будь-яких конкретних змін CSI.

#### Ефемерні томи CSI {#csi-ephemeral-volumes}

{{< feature-state for_k8s_version="v1.25" state="stable" >}}

Ви можете безпосередньо налаштувати томи CSI в межах специфікації Pod. Такі томи є ефемерними і не зберігаються після перезапуску Podʼів. Див. [Ефемерні томи](/uk/docs/concepts/storage/ephemeral-volumes/#csi-ephemeral-volumes) для отримання додаткової інформації.
...
Рекурсивне монтування тільки для читання може бути увімкнено встановленням [feature gate](/uk/docs/reference/command-line-tools-reference/feature-gates/) `RecursiveReadOnlyMounts` для kubelet та kube-apiserver, і встановленням поля `.spec.containers[].volumeMounts[].recursiveReadOnly` для Podʼа.
...
* `Enabled`: робить монтування рекурсивно тільки для читання. Потрібно задовольняти всі наступні вимоги:
  ...
  * Середовище виконання контейнерів рівня [CRI](/uk/docs/concepts/architecture/cri) підтримує рекурсивне монтування тільки для читання
...
Ознайомтесь з прикладом [розгортання WordPress та MySQL з Persistent Volume](/uk/docs/tutorials/stateful-application/mysql-wordpress-persistent-volume/).
