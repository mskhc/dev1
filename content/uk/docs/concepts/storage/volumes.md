...
Kubernetes підтримує багато типів томів. {{<glossary_tooltip term_id="pod" text="Pod">}} може використовувати одночасно будь-яку кількість типів томів. Типи [ефемеральних томів](/docs/concepts/storage/ephemeral-volumes/) існують протягом життєвого циклу Podʼу, але [постійні томи](/docs/concepts/storage/persistent-volumes/) існують поза життєвим циклом Podʼу. Коли Pod припиняє існування, Kubernetes знищує ефемеральні томи; проте Kubernetes не знищує постійні томи. Для будь-якого виду тому в певному Podʼі дані зберігаються при перезапуску контейнера.
...
### awsElasticBlockStore (вилучено) {#awselasticblockstore}
...
Тип тому `awsElasticBlockStore` не включено в Kubernetes {{< skew currentVersion >}}.
...
### azureDisk (вилучено) {#azuredisk}

<!-- Примітка про обслуговування: Дозволено вилучати всі вказівки про azureDisk, якщо випуск v1.27
Kubernetes перестав підтримуватися -->

Тип тому `azureDisk` не включено в Kubernetes {{< skew currentVersion >}}.
...
{{< note >}}
Якщо включено [feature gate](/docs/reference/command-line-tools-reference/feature-gates/) `SizeMemoryBackedVolumes`, можна вказати розмір для томів, які використовують памʼять. Якщо розмір не вказано, томи, які використовують памʼять, масштабуються до обсягу памʼяті, доступного для вузла.
{{< /note >}}
...
### gcePersistentDisk (вилучено) {#gcepersistentdisk}

Тип тому `gcePersistentDisk` не включено в Kubernetes {{< skew currentVersion >}}.
...
Функціональність `CSIMigration` для `RBD`, коли вона увімкнена, перенаправляє всі операції втулка з наявного вбудованого втулка на драйвер {{< glossary_tooltip text="CSI" term_id="csi" >}} `rbd.csi.ceph.com`. Щоб використовувати цю функцію, драйвер [Ceph CSI](https://github.com/ceph/ceph-csi) повинен бути встановлений в кластері Kubernetes, а [feature gate](/docs/reference/command-line-tools-reference/feature-gates/) `CSIMigrationRBD` повинен бути увімкнений (Зауважте, що у релізі v1.24 прапорець `csiMigrationRBD` був видалений і замінений на `CSIMigrationRBD`).
...
* Якщо ви використовуєте Kubernetes версії 1.25 або 1.26, вам потрібно увімкнути [feature gate](/docs/reference/command-line-tools-reference/feature-gates/) з назвою `CSINodeExpandSecret` для кожного kube-apiserver та для kubelet на кожному вузлі. У версії Kubernetes 1.27 цю функцію типово ввімкнено і не потрібно явно використовувати feature gate. Також вам потрібно використовувати драйвер CSI, який підтримує або вимагає конфіденційні дані secret під час операцій зміни розміру зберігання, ініційованих вузлом. 
