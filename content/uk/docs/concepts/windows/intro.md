...
* Привілейовані контейнери: не підтримуються для контейнерів Windows. [Контейнери HostProcess](/uk/docs/tasks/configure-pod-container/create-hostprocess-pod/) пропонують схожий функціонал.
...
* [Podsʼи](/uk/docs/concepts/workloads/pods/)
...
* [Ресурси робочих навантажень]](/uk/docs/concepts/workloads/controllers/):
  * ReplicaSet
  * Deployment
  * StatefulSet
  * DaemonSet
...
* {{< glossary_tooltip text="Service" term_id="service" >}}. Дивіться [Балансування навантаження та Service](/uk/docs/concepts/services-networking/windows-networking/#load-balancing-and-service) для деталей.
...
* `securityContext.privileged` — Windows не підтримує привілейовані контейнери, замість них використовуйте [Контейнери HostProcess](/uk/docs/tasks/configure-pod-container/create-hostprocess-pod/)
...
* `securityContext.runAsUser` — використовуйте [`runAsUserName`](/uk/docs/tasks/configure-pod-container/configure-runasusername) замість цього
...
Тільки поля `securityContext.runAsNonRoot` та `securityContext.windowsOptions` з поля Podʼа [`securityContext`](/uk/docs/reference/kubernetes-api/workload-resources/pod-v1/#security-context) працюють у Windows.

## Виявлення проблем вузла {#node-problem-detector}

Механізм виявлення проблем вузла (див. [Моніторинг справності вузла](/uk/docs/tasks/debug/debug-cluster/monitor-node-health/)) має попередню підтримку для Windows. Для отримання додаткової інформації відвідайте [сторінку проєкту на GitHub](https://github.com/kubernetes/node-problem-detector#windows).
...
Дізнайтеся, як [встановити ContainerD на вузол Windows](/uk/docs/setup/production-environment/container-runtimes/#containerd).
{{< note >}}
Є [відоме обмеження](/uk/docs/tasks/configure-pod-container/configure-gmsa/#gmsa-limitations) при використанні GMSA з containerd для доступу до мережевих ресурсів Windows, яке потребує
патча ядра.
{{< /note >}}
...
Також застосовується політика [відхилення версій](/uk/docs/setup/release/version-skew-policy/) Kubernetes.
...
Для отримання найновішої інформації про мінімальні апаратні вимоги дивіться [Вимоги апаратного забезпечення для Windows Server у документації Microsoft](https://learn.microsoft.com/en-us/windows-server/get-started/hardware-requirements). Для керівництва у виборі ресурсів для операційних робочих вузлів дивіться [Робочі вузли для операційного середовища в документації Kubernetes](/uk/docs/setup/production-environment/#production-worker-nodes).
...
Для усунення несправностей звертайтесь до відповідного [розділу](/uk/docs/tasks/debug/) цієї документації, він стане вашим головним джерелом в отриманні потрібних відомостей.
