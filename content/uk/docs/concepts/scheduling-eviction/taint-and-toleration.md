...
Ви додаєте позначку taint до вузла за допомогою [kubectl taint](/uk/docs/reference/generated/kubectl/kubectl-commands#taint). Наприклад,
...
Podʼи [DaemonSet](/uk/docs/concepts/workloads/controllers/daemonset/) створюються з толерантностями `NoExecute` для наступних taint без `tolerationSeconds`:
...
Панель управління, використовуючи {{<glossary_tooltip text="контролер" term_id="controller">}} вузла, автоматично створює taint з ефектом `NoSchedule` для [умов вузла](/uk/docs/concepts/scheduling-eviction/node-pressure-eviction/#node-conditions).
...
* Дізнайтеся про [Виселення через тиск на вузол](/uk/docs/concepts/scheduling-eviction/node-pressure-eviction/) та як його налаштувати
* Ознайомтесь з [Пріоритетом Podʼа](/uk/docs/concepts/scheduling-eviction/pod-priority-preemption/)
