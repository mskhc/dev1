...
`nodeSelector` є найпростішим рекомендованим способом обмеження вибору вузла. Ви можете додати поле `nodeSelector` до специфікації вашого Podʼу і вказати [мітки вузла](#built-in-node-labels), які має мати цільовий вузол. Kubernetes розміщує Pod лише на тих вузлах, які мають всі мітки, які ви вказали.
...
Крім `labelSelector` та `topologyKey`, ви можете опціонально вказати список просторів імен, з якими `labelSelector` повинен зіставлятися, використовуючи поле `namespaces` на тому ж рівні, що й `labelSelector` та `topologyKey`. Якщо відсутнє або порожнє, `namespaces` типово відноситься до простору імен Podʼа, де з'являється визначення спорідненості/антиспорідненості.
...
У наступному прикладі Deployment для вебсерверів створює репліки з міткою `app=web-store`. Правило спорідненості Podʼу повідомляє планувальнику розмістити кожну репліку на вузлі, де є Pod з міткою `app=store`. Правило антиспорідненості Podʼу повідомляє планувальнику ніколи не розміщати декілька серверів `app=web-store` на одному вузлі.
...
У вас можуть бути інші причини використання антиспорідненості Podʼу. Дивіться [посібник по ZooKeeper](/docs/tutorials/stateful-application/zookeeper/#tolerating-node-failure)
...
- Якщо зазначений вузол не має достатньо ресурсів для розміщення Podʼу, Pod зазнає збою, про що буде повідомлено,  наприклад, OutOfmemory або OutOfcpu.
...
|    Оператор    |    Поведінка    |
| :------------: | :-------------: |
| `Gt` | Значення поля буде розібране як ціле число, і це ціле число менше або дорівнює цілому числу, яке отримується при розборі значення мітки, названої цим селектором |
| `Lt` | Значення поля буде розібране як ціле число, і це ціле число більше або дорівнює цілому числу, яке отримується при розборі значення мітки, названої цим селектором |
