---
title: Вивільнення ресурсів внаслідок тиску на вузол
...
Під час вивільнення внаслідок тиску на вузол, kubelet встановлює [фазу](/docs/concepts/workloads/pods/pod-lifecycle/#pod-phase) для вибраних Podʼів в значення `Failed` та завершує роботу Podʼа.

Вивільнення внаслідок тиску на вузол, не є тим самим, що й [вивільнення, ініційоване API](/docs/concepts/scheduling-eviction/api-eviction/).

kubelet не дотримується вашого налаштованого {{<glossary_tooltip term_id="pod-disruption-budget" text="PodDisruptionBudget">}} або параметру `terminationGracePeriodSeconds` Podʼа. Якщо ви використовуєте [порогові значення мʼякого вивільнення](#soft-eviction-thresholds), kubelet дотримується вашого налаштованого `eviction-max-pod-grace-period`. Якщо ви використовуєте [порогові значення жорсткого вивільнення](#hard-eviction-thresholds), kubelet використовує граничний період `0s` (негайне завершення) для завершення роботи Podʼа.
...
На Linux kubelet використовує такі сигнали виселення:

| Сигнал виселення    | Опис                                                                                     |
|----------------------|------------------------------------------------------------------------------------------|
...
Kubelet визнає два конкретні ідентифікатори файлових систем:

1. `nodefs`: Основна файлова система вузла, яка використовується для локальних дискових томів, томів emptyDir, що не підтримуються памʼяттю, зберігання журналів та інше. Наприклад, `nodefs` містить `/var/lib/kubelet/`.
2. `imagefs`: Необовʼязкова файлова система, яку середовище виконання контейнерів використовують для зберігання образів контейнерів та шарів контейнерів, що мають можливість для запису.

Kubelet автоматично виявляє ці файлові системи та ігнорує інші локальні файлові системи вузла. Kubelet не підтримує інші конфігурації.
...
Ви можете вказати спеціальні пороги виселення для kubelet, які він буде використовувати під час прийняття рішень про виселення. Ви можете налаштувати [м'які](#soft-eviction-thresholds) та [жорсткі](#hard-eviction-thresholds) пороги виселення.
...
| Стан Вузла         | Сигнал Виселення                                                                       | Опис                                                                                                                        |
|-------------------|---------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------|
| `MemoryPressure`  | `memory.available`                                                                    | Доступна памʼять на вузлі задовольнила поріг виселення                                                                      |
| `DiskPressure`    | `nodefs.available`, `nodefs.inodesFree`, `imagefs.available`, or `imagefs.inodesFree` | Доступний дисковий простір та inode на основній файловій системі вузла або файловій системі образу задовольнили поріг виселення |
| `PIDPressure`     | `pid.available`                                                                       | Доступні ідентифікатори процесів на вузлі (Linux) впали нижче порогу виселення                                               |
...
#### Без використання `imagefs` {#without-imagefs}

Якщо на вузлі є лише файлова система `nodefs`, яка відповідає пороговим значенням для виселення, kubelet звільняє місце на диску в наступному порядку:

1. Збір сміття мертвих Podʼів та контейнерів
2. Видалення невикористаних образів
...
1. Гарантовані Podʼи та Podʼи `Burstable`, де використання менше за запити, виселяються останніми, на основі їх пріоритету.
...
#### З `imagefs` {#with-imagefs-1}

Якщо виселення викликається `nodefs`, kubelet сортує Podʼи на основі використання `nodefs` (`локальні томи + журнали всіх контейнерів`).

Якщо виселення викликається `imagefs`, kubelet сортує Podʼи на основі використання шару для запису всіх контейнерів.

#### Без `imagefs` {#without-imagefs-1}

Якщо виселення викликається `nodefs`, kubelet сортує Podʼи на основі їхнього загального використання диска (`локальні томи + журнали та шар для запису всіх контейнерів`).
