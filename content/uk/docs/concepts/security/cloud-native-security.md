...
6. Обмежуйте доступ до артефактів. Розміщуйте образи контейнерів у [приватному реєстрі](/uk/docs/concepts/containers/images/#using-a-private-registry), який дозволяє тільки авторизованим клієнтам отримувати образи.
...
Інші сторінки документації Kubernetes містять більше деталей щодо того, як налаштувати конкретні аспекти контролю доступу. [Перелік перевірок безпеки](/uk/docs/concepts/security/security-checklist/) містить набір рекомендованих базових перевірок для вашого кластера.

Поза цим, захист вашого кластера означає впровадження ефективних [способів автентифікації](/uk/docs/concepts/security/controlling-access/#authentication) та [авторизації](/uk/docs/concepts/security/controlling-access/#authorization) для доступу до API. Використовуйте [ServiceAccounts](/uk/docs/concepts/security/service-accounts/), щоб забезпечити та керувати перевірками безпеки для робочих навантажень та компонентів кластера.

Kubernetes використовує TLS для захисту трафіку API; переконайтеся, що кластер розгорнуто за допомогою TLS (включаючи трафік між вузлами та панеллю управління), та захистіть ключі шифрування. Якщо ви використовуєте власний API Kubernetes для [CertificateSigningRequests](/uk/docs/reference/access-authn-authz/certificate-signing-requests/#certificate-signing-requests), приділіть особливу увагу обмеженню зловживань там.
...
1. Застосовувати [стандарти безпеки для Pod](/uk/docs/concepts/security/pod-security-standards/) для застосунків, щоб забезпечити їх запуск лише з необхідними привілеями.
...
3. Визначати [ResourceQuotas](/uk/docs/concepts/policy/resource-quotas/) для справедливого розподілу спільних ресурсів та використовувати механізми, такі як [LimitRanges](/uk/docs/concepts/policy/limit-range/) для забезпечення того, що Podʼи вказують свої вимоги до ресурсів.
4. Розподіляти робочі навантаження по різних вузлах. Використовуйте механізми [ізоляції вузлів](/uk/docs/concepts/scheduling-eviction/assign-pod-node/#node-isolation-restriction), як від самого Kubernetes, так і від екосистеми, щоб гарантувати, що Pod з різними контекстами довіри виконуються на окремих наборах вузлів.
...
6. На вузлах Linux використовуйте модуль безпеки Linux, такий як [AppArmor](/uk/docs/tutorials/security/apparmor/) або [seccomp](/uk/docs/tutorials/security/seccomp/).
...
2. Увімкніть [шифрування в стані спокою](/uk/docs/tasks/administer-cluster/encrypt-data/) для обʼєктів API.
...
- [Безпека Kubernetes](/uk/docs/concepts/security/)
- [Забезпечення безпеки вашого кластера](/uk/docs/tasks/administer-cluster/securing-a-cluster/)
- [Шифрування даних під час передачі](/uk/docs/tasks/tls/managing-tls-in-a-cluster/) для площини керування
- [Шифрування даних в стані спокою](/uk/docs/tasks/administer-cluster/encrypt-data/)
...
- [Мережеві політики](/uk/docs/concepts/services-networking/network-policies/) для Pod-ів
- [Стандарти безпеки для Pod-ів](/uk/docs/concepts/security/pod-security-standards/)
