...
- **managedFields** ([]ManagedFieldsEntry)
  ...
  *ManagedFieldsEntry — це ідентифікатор робочого процесу, набір полів (FieldSet) і версія групи ресурсу, до якого застосовується набір полів.*
  ...
  - **managedFields.time** (Time)
    ...
    <a name="Time"></a>
    *Time — це обгортка навколо time.Time, яка підтримує правильне перетворення в YAML і JSON. Для багатьох методів створення, що пропонує пакет time, надаються обгортки.*

  - **ownerReferences** ([]OwnerReference)

    *Стратегія патча: злиття за ключем `uid`*
