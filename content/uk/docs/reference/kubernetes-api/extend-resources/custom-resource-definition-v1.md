...
- **x-kubernetes-validations**
  ...
  *Map: унікальні значення ключа rule будуть збережені під час злиття.*
  ...
  x-kubernetes-validations описує список правил валідації записаних мовою CEL. Це полн є полем рівня альфа. Використання цього поля вимагає увімкнення функціональної можливості `CustomResourceValidationExpressions`.
  ...
  *ValidationRule описує правило валідації записане мовою CEL.*
  ...
  - **x-kubernetes-validations.rule** (string), обовʼязково
    ...
    - Правило обмежене коренем ресурсу з підресурсом status: {"rule": "self.status.actual \<= self.spec.maxDesired"}
