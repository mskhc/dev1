...
За умови, що ввімкнено [функціонал](/uk/docs/reference/command-line-tools-reference/feature-gates/) `WatchList`, це можна зробити, вказавши `sendInitialEvents=true` як параметр рядка запиту у запиті **watch**. Якщо встановлено, сервер API починає потік спостереження з синтетичних початкових подій (типу `ADDED`) для побудови всього стану всіх наявних обʼєктів, після чого йде подія [`BOOKMARK`](/uk/docs/reference/using-api/api-concepts/#watch-bookmarks) (якщо запитано через параметр `allowWatchBookmarks=true`). Подія закладки включає версію ресурсу, до якої його було синхронізовано. Після надсилання події закладки сервер API продовжує роботу як для будь-якого іншого запиту **watch**.
...
Стиснення відповідей API може значно зменшити розмір відповіді, особливо для великих ресурсів або [колекцій](/uk/docs/reference/using-api/api-concepts/#collections). Наприклад, запит **list** для Podʼів може повернути сотні кілобайт або навіть мегабайти даних, залежно від кількості Podʼів та їх атрибутів. Стиснення відповіді дозволяє зберегти мережеву пропускну здатність та зменшити затримки.
...
Є десятки типів колекцій (таких як `PodList`, `ServiceList` та `NodeList`), визначених в API Kubernetes. Ви можете отримати більше інформації про кожен тип колекції з довідника [Kubernetes API](/uk/docs/reference/kubernetes-api/).
...
Якщо версія запиту без dry-run викликала б контролер доступу, який має побічні ефекти, запит буде відхилений, щоб уникнути небажаних побічних ефектів. Усі вбудовані втулки контролю доступу підтримують dry-run. Додатково, admission webhooks можуть оголосити у своїй [конфігураційній моделі](/uk/docs/reference/generated/kubernetes-api/{{< param "version" >}}/#validatingwebhook-v1-admissionregistration-k8s-io), що вони не мають побічних ефектів, встановивши поле `sideEffects` на `None`.
...
* `UID`: [унікально ідентифікує](/uk/docs/concepts/overview/working-with-objects/names/#uids) обʼєкт і генерується випадково (недетерміновано)
...
Дивіться [Огляд авторизації](/uk/docs/reference/access-authn-authz/authorization/).
...
Дивіться [Серверне застосування](/uk/docs/reference/using-api/server-side-apply/) для отримання додаткової інформації.
...
[v1.meta/ObjectMeta](/uk/docs/reference/generated/kubernetes-api/{{< param "version" >}}/#objectmeta-v1-meta) — `metadata.resourceVersion` екземпляра ресурсу ідентифікує версію ресурсу, на якій останній раз він був змінений.

[v1.meta/ListMeta](/uk/docs/reference/generated/kubernetes-api/{{< param "version" >}}/#listmeta-v1-meta) — `metadata.resourceVersion` колекції ресурсів (відповідь на **перелік (list)**) ідентифікує версію ресурсу, на якій була створена колекція.
