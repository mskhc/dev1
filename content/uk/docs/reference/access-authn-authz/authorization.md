...
Авторизація в Kubernetes відбувається після [автентифікації](/uk/docs/reference/access-authn-authz/authentication/). Зазвичай клієнт, що робить запит, має бути автентифікований (увійти в систему), перш ніж його запит може бути дозволений; однак, Kubernetes також дозволяє анонімні запити за деяких обставин.

Для огляду того, як авторизація вписується в ширший контекст контролю доступу до API, читайте [Контроль доступу до Kubernetes API](/uk/docs/concepts/security/controlling-access/).
...
* **API group** — {{< glossary_tooltip text="Група API" term_id="api-group" >}}, до якої здійснюється доступ (тільки для запитів до ресурсів). Порожній рядок позначає _основну_ [групу API](/uk/docs/reference/using-api/#api-groups).
...
* Особливі випадки [автентифікації](/uk/docs/reference/access-authn-authz/authentication/)
...
* [Авторизація CertificateSigningRequests](/uk/docs/reference/access-authn-authz/certificate-signing-requests/#authorization)
  * Дієслово **approve** для CertificateSigningRequests, та **update** для переглядів наявних схвалень
* [RBAC](/uk/docs/reference/access-authn-authz/rbac/#privilege-escalation-prevention-and-bootstrapping)
...
`ABAC` ([контроль доступу на основі атрибутів](/uk/docs/reference/access-authn-authz/abac/))
...
`RBAC` ([контроль доступу на основі ролей](/uk/docs/reference/access-authn-authz/rbac/))
...
`Node`
: Спеціальний режим авторизації, який надає дозволи для kubeletʼів на основі запланованих до запуску Podʼів. Щоб дізнатися більше про режим авторизації вузла, див. [Авторизація вузла](/uk/docs/reference/access-authn-authz/node/).

`Webhook`
: Kubernetes [режим webhook](/uk/docs/reference/access-authn-authz/webhook/) для авторизації робить синхронний HTTP-виклик, блокуючи запит до тих пір, поки віддалений HTTP-сервіс не відповість на нього. Ви можете написати власне програмне забезпечення для обробки виклику або використовувати рішення з екосистеми.
...
Для отримання додаткової інформації про аргументи командного рядка для API сервера, читайте [довідник по `kube-apiserver`](/uk/docs/reference/command-line-tools-reference/kube-apiserver/).
...
Користувачі, які можуть створювати/редагувати Podʼи в просторі імен, або безпосередньо, або через обʼєкт, що дозволяє опосередковане [управління робочими навантаженнями](/uk/docs/concepts/architecture/controller/), можуть мати можливість підвищити свої привілеї в цьому просторі імен. Потенційні шляхи до підвищення привілеїв включають розширення [API Kubernetes](/uk/docs/concepts/extend-kubernetes/#api-extensions) та повʼязані з ними {{< glossary_tooltip term_id="controller" text="контролери" >}}.
...
Адміністратори можуть поєднувати це з [імперсонізацією користувача](/uk/docs/reference/access-authn-authz/authentication/#user-impersonation), щоб визначити, які дії можуть виконувати інші користувачі.
...
* Щоб дізнатися більше про автентифікацію, перегляньте [Автентифікацію](/uk/docs/reference/access-authn-authz/authentication/).
* Для отримання огляду, перегляньте [Керування доступом до API Kubernetes](/uk/docs/concepts/security/controlling-access/).
* Щоб дізнатися більше про контроль прийняття, перегляньте [Використання контролерів допуску](/uk/docs/reference/access-authn-authz/admission-controllers/).
* Дізнайтесь про [Загальну мову запитів (CEL) в Kubernetes](/uk/docs/reference/using-api/cel/).
