...
- Всі статичні Podʼи отримують будь-які додаткові прапорці, вказані користувачем, як описано в [передача користувацьких аргументів компонентам панелі управління](/docs/setup/production-environment/tools/kubeadm/control-plane-flags/)
...
- Сервіс CoreDNS називається `kube-dns`. Це зроблено для запобігання будь-яких перерв у роботі, коли користувач переключає DNS кластера з kube-dns на CoreDNS за допомогою методу `--config`, описаного [тут](/docs/reference/setup-tools/kubeadm/kubeadm-init-phase/#cmd-phase-addon).
