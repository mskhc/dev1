...
Для отримання додаткової інформації про поля та використання конфігурації ви можете перейти на нашу [сторінку API-довідки](/uk/docs/reference/config-api/kubeadm-config.v1beta3/).
...
Щоб передати функціональні можливості, можна використовувати прапорець `--feature-gates` для `kubeadm init`, або можна додати елементи до поля `featureGates`, коли передаєте [конфігураційний файл](/uk/docs/reference/config-api/kubeadm-config.v1beta3/#kubeadm-k8s-io-v1beta3-ClusterConfiguration) за допомогою `--config`.
...
: Ця **вимкнута** функціональна можливість, яка була введена в Kubernetes v1.28 для повторної активації застарілої поведінки під час оновлення кластера. Для версій kubeadm до v1.28, kubeadm оновлює надбудови кластера (включаючи CoreDNS та kube-proxy) негайно під час `kubeadm upgrade apply`, незалежно від того, чи були інші екземпляри панелі управління оновлені. Це може викликати проблеми сумісності. Починаючи з v1.28, kubeadm стандартно працює у режимі, який завжди перевіряє, чи всі екземпляри панелі управління були оновлені перед початком оновлення надбудов. Ця поведінка застосовується як до `kubeadm upgrade apply`, так і до `kubeadm upgrade node`. kubeadm визначає, чи було оновлено екземпляр панелі управління, перевіряючи, чи було оновлено образ Pod kube-apiserver. Ви повинні виконувати оновлення екземпляра панелі управління послідовно або принаймні переконатися, що останнє оновлення екземпляра панелі управління не розпочато, поки всі інші екземпляри панелі управління не будуть повністю оновлені, а оновлення надбудов буде виконане після останнього оновлення екземпляра панелі управління. Застаріла функціональна можливість `UpgradeAddonsBeforeControlPlane` дає вам можливість зберегти стару поведінку оновлення. Вам не потрібно використовувати цю стару поведінку; якщо потрібно, розгляньте можливість змінити ваш кластер або процеси оновлення, оскільки ця функціональна можливість буде видалена в майбутньому випуску.
...
{{< note >}}
Попередньо визначений `certificateKey` можна вказати в `InitConfiguration` при передачі [файлу конфігурації](/uk/docs/reference/config-api/kubeadm-config.v1beta3/) за допомогою `--config`.
{{< /note >}}
