...
- Використання прапорця `--config` команди `kubeadm upgrade` з [типами конфігурації kubeadm](/uk/docs/reference/config-api/kubeadm-config.v1beta3) з метою переконфігурування кластера не рекомендується і може мати неочікувані наслідки. Дотримуйтеся кроків з [Переконфігурація кластера kubeadm](/uk/docs/tasks/administer-cluster/kubeadm/kubeadm-reconfigure) замість цього.
