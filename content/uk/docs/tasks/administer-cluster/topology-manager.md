...
{{< note >}}
Щоб вирівняти ресурси ЦП з іншими запитаними ресурсами у специфікації Pod, потрібно включити CPU Manager та налаштувати відповідну політику CPU Manager на вузлі. Див. [керування політиками керування ЦП](/docs/tasks/administer-cluster/cpu-management-policies/).
{{< /note >}}
...
Можна вибрати будь-яку опцію при запуску kubelet, вказавши прапорець `--topology-manager-scope`.

### Scope контейнера {#container-scope}

Scope `container` використовується стандартно.
...
Для вибору scope `pod` запустіть kubelet з опцією командного рядка `--topology-manager-scope=pod`.
...
Загальна кількість певного ресурсу, запитаного для всього Podʼу, розраховується за формулою [ефективні запити/обмеження](/docs/concepts/workloads/pods/init-containers/#resources), і, таким чином, це загальне значення дорівнює максимуму з:
...
Підтримка параметрів політики керування топологією вимагає активації [feature gate](/docs/reference/command-line-tools-reference/feature-gates/) `TopologyManagerPolicyOptions`, щоб бути ввімкненими (типово вимкнено).
...
Існують наступні параметри політики:

- `prefer-closest-numa-nodes` (бета, стандартно видимий; опції `TopologyManagerPolicyOptions` та `TopologyManagerPolicyBetaOptions` повинні бути ввімкнені). Опція політики `prefer-closest-numa-nodes` є бета-версією в Kubernetes {{< skew currentVersion >}}.

Якщо вказано параметр політики `prefer-closest-numa-nodes`, політики `best-effort` та `restricted` будуть віддавати перевагу наборам вузлів NUMA з коротшими відстанями між ними при прийнятті рішень про прийняття. Ви можете увімкнути цей параметр, додавши `prefer-closest-numa-nodes=true` до параметрів політики Topology Manager. Стандартно, без цієї опції, Topology Manager вирівнює ресурси або на одному вузлі NUMA, або мінімальна кількість вузлів NUMA (у випадках, коли потрібно більше одного вузла NUMA). Однак, `TopologyManager` не враховує відстані між NUMA-вузлами при прийнятті рішень про прийняття. Це обмеження виявляється в багатопроцесорних, а також однопроцесорних системах з кількома NUMA-вузлами, і може призвести до значного погіршення продуктивності в випадку виконання завдань з високими вимогами до часу виконання та великим обсягом обчислень, якщо Topology Manager вирішить вирівняти ресурси на неприлеглих NUMA-вузлах.

### Взаємодія кінцевих точок Pod з політиками Topology Manager {#pod-interaction-with-topology-manager-policies}

Розгляньте контейнери у наступних специфікаціях Pod:
...
1. Максимальна кількість вузлів NUMA, яку дозволяє Topology Manager, — 8. З більш ніж 8 вузлами NUMA відбудеться вибух стану при спробі перерахувати можливі спорідненості NUMA та генерувати їх підказки.
