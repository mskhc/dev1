...
Починаючи з версії v1.22, Менеджер памʼяті типово увімкнено за допомоги `MemoryManager` [feature gate](/docs/reference/command-line-tools-reference/feature-gates/).
...
Новий прапор `--reserved-memory` було додано до Memory Manager, щоб дозволити цю загальну зарезервовану памʼять розділити (адміністратором вузла) і відповідно зарезервувати для багатьох вузлів NUMA.
...
Kubelet надає службу gRPC `PodResourceLister` для включення виявлення ресурсів та повʼязаних метаданих. Використовуючи його [кінцеву точку List gRPC](/docs/concepts/extend-kubernetes/compute-storage-net/device-plugins/#grpc-endpoint-list), можна отримати інформацію про зарезервовану памʼять для кожного контейнера, яка міститься у protobuf повідомленні `ContainerMemory`. Цю інформацію можна отримати лише для Podʼів у класі якості обслуговування Guaranteed.
