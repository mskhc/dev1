...
Підмножина конфігурації kubelet, яку можна налаштувати через файл, визначається структурою [`KubeletConfiguration`](/uk/docs/reference/config-api/kubelet-config.v1beta1/).
...
{{< note >}}
Якщо ви використовуєте kubeadm для ініціалізації кластера, використовуйте kubelet-config під час створення вашого кластера за допомогою `kubeadm init`. Дивіться [налаштування kubelet за допомогою kubeadm](/uk/docs/setup/production-environment/tools/kubeadm/kubelet-integration/) для деталей.
{{< /note >}}
...
Різні типи даних у структурі конфігурації kubelet обʼєднуються по-різному. Дивіться [посилання на довідку](/uk/docs/reference/node/kubelet-config-directory-merging/) для отримання додаткової інформації.
...
1. Запустіть проксі-сервер за допомогою команди [`kubectl proxy`](/uk/docs/reference/kubectl/generated/kubectl-commands#proxy) у вашому терміналі.
...
- Дізнайтеся більше про конфігурацію kubelet, переглянувши
  [довідник `KubeletConfiguration`](/uk/docs/reference/config-api/kubelet-config.v1beta1/).
- Дізнайтеся більше про обʼєднання конфігурації kubelet у [довідці](/uk/docs/reference/node/kubelet-config-directory-merging).
