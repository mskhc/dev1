...
{{< note >}}
Конвертація за допомогою вебхука доступна як бета-версія з версії 1.15, і як альфа-версія з Kubernetes 1.13. Функція `CustomResourceWebhookConversion` повинна бути увімкнена, що автоматично відбувається для багатьох кластерів з бета-функціями. Будь ласка, зверніться до документації [feature gate](/docs/reference/command-line-tools-reference/feature-gates/) для отримання додаткової інформації.
{{< /note >}}
...
{{< note >}}
Коли сервер вебхука розгорнуто в кластер Kubernetes як сервіс, він має бути доступний через сервіс на порту 443 (сам сервер може використовувати довільний порт, але об'єкт сервісу повинен зіставити його з портом 443). Комунікація між API-сервером і сервісом вебхука може не вдатися, якщо використовується інший порт для сервісу.
{{< /note >}}
...
```yaml
{
  ...
  "request": {
    ...
    # Список обʼєктів для конвертації.
    # Може містити один або більше об'єктів, у одній або більше версіях.
    "objects": [
      {
        "kind": "CronTab",
        "apiVersion": "example.com/v1beta1",
...
{{< note >}}
Прапорець `--subresource` використовується з командами kubectl get, patch, edit та replace для отримання та оновлення підресурсів, таких як `status` і `scale`, для всіх ресурсів API, які їх підтримують. Цей прапорець доступний, починаючи з версії kubectl v1.24. Раніше читання підресурсів (наприклад, `status`) через kubectl передбачало використання `kubectl --raw`, а оновлення підресурсів за допомогою kubectl взагалі було неможливим. Починаючи з v1.24, інструмент `kubectl` можна використовувати для редагування або накладання патчів на субресурс `status` обʼєкта CRD. Див. [Як патчити Deployment за допомогою прапорця subresource](/docs/tasks/manage-kubernetes-objects/update-api-object-kubectl-patch/#scale-kubectl-patch).
