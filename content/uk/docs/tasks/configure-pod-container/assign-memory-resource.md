---
title: Виділення ресурсів памʼяті для контейнерів та Pod'ів
content_type: task
weight: 10
---
...
Ця сторінка показує, як вказати *запити* та *ліміти* пам'яті для контейнерів. Контейнери гарантовано матимуть стільки памʼяті, скільки вказано у запиті, і не отримають більше, ніж вказано у ліміті.
...
## Визначення запитів та лімітів пам'яті {#specify-a-memory-request-and-a-memory-limit}
...
Вивід містить запис про знищення Контейнера через умову вичерпання пам'яті:
...
Запити та ліміт памʼяті повʼязані з Контейнерами, але корисно думати про Pod як про елемент, що має запит та ліміт пам'яті. Запит пам'яті для Podʼа — це сума запитів памʼяті для всіх Контейнерів у Podʼі. Аналогічно, ліміт памʼяті для Podʼа — це сума лімітів всіх Контейнерів у Podʼі.
...
Налаштовуючи запити та ліміти пам'яті для Контейнерів, які працюють у вашому
кластері, ви можете ефективно використовувати ресурси пам'яті, доступні на вузлах вашого кластера. Знижуючи запит памʼяті для Podʼа, ви даєте Podʼу хороший шанс на планування. Маючи ліміти памʼяті, який перевищує запит памʼяті, ви досягаєте двох цілей:
