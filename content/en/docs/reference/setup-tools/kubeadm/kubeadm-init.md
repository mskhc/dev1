...
`NodeLocalCRISocket`
: With this feature gate enabled, kubeadm will consume the CRI socket for each node from the file
`/var/lib/kubelet/instance-config.yaml` instead of writing it as the annotation
`kubeadm.alpha.kubernetes.io/cri-socket` on the Node object. The new file is applied as an instance
configuration patch, before any other user managed patches are applied when the `--patches` functionality
is used.
